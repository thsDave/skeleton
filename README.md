...
- Eventos auditados principales: `sessions.created`, `sessions.revoked_by_user`, `sessions.revoked_by_admin`, `sessions.revoked_others`, `sessions.revoked_all_user`, `sessions.revoked_due_password_change`, `sessions.revoked_due_password_reset`, `sessions.revoked_due_required_password_change`, `sessions.revoked_due_mfa_change`, `sessions.revoked_due_email_change`, `sessions.revoked_due_admin_password_change`, `sessions.revoked_due_force_password_change` y `sessions.revoked_detected_on_request`.
- Acciones sensibles que cierran otras sesiones manteniendo la sesion actual: cambio de contrasena propio, cambio obligatorio de contrasena, cambio de correo completado y activacion/desactivacion de MFA.
- Acciones sensibles que cierran todas las sesiones del usuario afectado: restablecimiento de contrasena por correo, cambio de contrasena realizado por administrador y activacion de `force_password_change` por administrador. Si el administrador actua sobre su propia cuenta, se mantiene la sesion actual.
- Motivos visibles en historial: `password_changed`, `password_reset`, `required_password_change`, `mfa_changed`, `email_changed`, `admin_password_changed`, `force_password_change` y `admin_mfa_reset`.
